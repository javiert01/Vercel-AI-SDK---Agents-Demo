...
export default function AgentPage() {
  ...
  async function handleClick() {
    ...
    const response = await fetch("/api/agent-calculator", {
      method: "POST",
  ...
  return (
    <div className="p-8">
      <h1 className="text-2xl font-bold mb-4">My first agent</h1>
      <div className="flex items-center gap-4 mb-4">
        <input
          type="text"
          value={prompt}
          onChange={(e) => setPrompt(e.target.value)}
          placeholder="Enter your prompt, e.g. 'Which is the result of 345 - 40?'"
        ...
        >
          {isStreaming ? "Streaming..." : "Calculate Result"}
        </button>
      </div>
